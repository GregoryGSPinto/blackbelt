...
export function StepKids({ kids, onAddKid, onRemoveKid, onContinue, error, setError }: StepKidsProps) {
  ...
  const handleContinue = () => {
    if (querKids === null) { setError(t('registerKids.chooseOption')); return; }
    if (querKids && kids.length === 0) {
      setError(t('registerKids.addAtLeastOne')); return;
    }
    setError('');
    onContinue();
  };
    ...
    <div className="space-y-6">
      <div className="p-4 bg-blue-600/10 border border-blue-600/30 rounded-xl">
        <div className="flex gap-3">
          ...
          <div>
            <p className="text-sm text-blue-300 font-medium mb-1">{t('registerKids.addKidsTitle')}</p>
            <p className="text-xs text-blue-300/80">{t('registerKids.addKidsDesc')}</p>
          </div>
        </div>
      </div>
        ...
        <>
              ...
              {kids.map((k, i) => (
                <div key={i} className="p-4 bg-white/5 rounded-xl border border-white/10 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-white/60">{t('registerKids.yearsOld', { age: calcIdade(k.dataNascimento) })}</p>
                  </div>
          ...
          <div className="space-y-4 p-6 bg-white/5 rounded-xl border border-white/10">
            <p className="text-sm font-medium mb-3">
              {kids.length === 0 ? t('registerKids.addFirstChild') : t('registerKids.addAnotherChild')}
            </p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {([
                ...
                { value: 'feminino', label: t('registerKids.girl') },
                { value: 'nao-informar', label: t('registerKids.notInformed') }
              ] as const).map(o => (
