import { NextRequest } from 'next/server';
import { withAuth, apiOk, apiError, apiServerError } from '@/lib/api/route-helpers';

export const dynamic = 'force-dynamic';

export async function POST(req: NextRequest) {
  try {
    const { supabase, membership } = await withAuth(req);
    if (!membership) return apiError('Sem membership ativa', 'NO_MEMBERSHIP');

    const body = await req.json();
    const { alunoId, turmaId, method = 'MANUAL' } = body;

    if (!alunoId) return apiError('alunoId é obrigatório', 'VALIDATION');

    // Verify the student exists and is active in this academy
    const { data: studentMember, error: memberErr } = await supabase
      .from('memberships' as any)
      .select('id, profile_id, role, status, belt_rank')
      .eq('profile_id', alunoId)
      .eq('academy_id', membership.academy_id)
      .single();

    if (memberErr || !studentMember) {
      return apiError('Aluno não encontrado nesta academia', 'NOT_FOUND', 404);
    }

    // Get profile info separately
    const { data: profile } = await supabase
      .from('profiles' as any)
      .select('full_name, avatar_url')
      .eq('id', alunoId)
      .single();

    if (studentMember.status !== 'active') {
      return apiOk({
        success: false,
        error: `Aluno com status: ${studentMember.status}`,
        aluno: {
          id: studentMember.profile_id,
          nome: profile?.full_name || '',
          graduacao: studentMember.belt_rank,
          status: studentMember.status,
        },
      });
    }

    // ── Enrollment validation: verify student is enrolled in the specified class ──
    // The student must have an active membership AND the class must belong to the same academy.
    if (turmaId) {
      const { data: schedule, error: schedErr } = await supabase
        .from('class_schedules' as any)
        .select('id, academy_id, active')
        .eq('id', turmaId)
        .single();

      if (schedErr || !schedule) {
        return apiError('Turma não encontrada', 'NOT_FOUND', 404);
      }

      if (schedule.academy_id !== membership.academy_id) {
        return apiError('Aluno não pertence a esta academia/turma', 'FORBIDDEN', 403);
      }

      if (!schedule.active) {
        return apiError('Turma não está ativa', 'INACTIVE_CLASS');
      }
    }

    // Find or create today's session
    const today = new Date().toISOString().split('T')[0];
    let sessionId: string;

    if (turmaId) {
      const { data: session } = await supabase
        .from('class_sessions' as any)
        .select('id')
        .eq('schedule_id', turmaId)
        .eq('date', today)
        .maybeSingle();

      if (session) {
        sessionId = session.id;
      } else {
        // Get instructor_id from the schedule
        const { data: schedule } = await supabase
          .from('class_schedules' as any)
          .select('instructor_id')
          .eq('id', turmaId)
          .single();

        const { data: newSession } = await supabase
          .from('class_sessions' as any)
          .insert({
            schedule_id: turmaId,
            academy_id: membership.academy_id,
            date: today,
            status: 'scheduled',
            instructor_id: schedule?.instructor_id || membership.id,
          })
          .select('id')
          .single();
        sessionId = newSession!.id;
      }
    } else {
      // Create a walk-in session if no turmaId specified
      // Get any active schedule for a generic session
      const { data: anySchedule } = await supabase
        .from('class_schedules' as any)
        .select('id, instructor_id')
        .eq('academy_id', membership.academy_id)
        .eq('active', true)
        .limit(1)
        .single();

      if (!anySchedule) {
        return apiError('Nenhuma turma ativa encontrada', 'NO_CLASS');
      }

      const { data: walkInSession } = await supabase
        .from('class_sessions' as any)
        .insert({
          schedule_id: anySchedule.id,
          academy_id: membership.academy_id,
          date: today,
          status: 'scheduled',
          instructor_id: anySchedule.instructor_id,
        })
        .select('id')
        .single();
      sessionId = walkInSession!.id;
    }

    // Create attendance record
    const { data: attendance, error: attErr } = await supabase
      .from('attendances' as any)
      .insert({
        session_id: sessionId,
        membership_id: studentMember.id,
        academy_id: membership.academy_id,
        checkin_method: method,
      })
      .select()
      .single();

    if (attErr) throw attErr;

    // Award gamification points (fire-and-forget)
    supabase.from('points_ledger' as any).insert({
      membership_id: studentMember.id,
      academy_id: membership.academy_id,
      points: 10,
      reason: 'Check-in realizado',
      reference_type: 'attendance',
      reference_id: attendance.id,
    }).then(() => {});

    return apiOk({
      success: true,
      checkIn: {
        id: attendance.id,
        alunoId: studentMember.profile_id,
        alunoNome: profile?.full_name || '',
        turmaId: turmaId || '',
        turmaNome: '',
        dataHora: attendance.checked_in_at,
        status: 'confirmado',
        method,
      },
      aluno: {
        id: studentMember.profile_id,
        nome: profile?.full_name || '',
        avatar: profile?.avatar_url,
        graduacao: studentMember.belt_rank,
        status: 'ATIVO',
      },
    });
  } catch (err) {
    if (err instanceof Response) return err;
    return apiServerError(err);
  }
}
