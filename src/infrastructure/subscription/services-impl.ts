...
export class OverageBillingService {
  ...
  async applyPrepaidCredits(academyId: string): Promise<{
    ...
    for (const quota of quotas) {
      ...
      const relevantCredits = credits.filter((c: any) => 
  ...
  async generateInvoice(
    ...
    const addonsAmount = addons?.reduce((sum: any, a: any) => sum + a.price, 0) || 0;
...
export class StoreRevenueService {
  ...
  async calculateMinimumFee(academyId: string, month: Date): Promise<{
    ...
    const totalSales = transactions?.reduce((sum: any, t: any) => sum + t.transaction_amount, 0) || 0;
  ...
  async processTransactions(
    ...
    const totalNetAmount = transactions.reduce((sum: any, t: any) => sum + t.net_amount, 0);
